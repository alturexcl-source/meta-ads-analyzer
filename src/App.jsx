...
const S = {
  ...
  btn: (c = PALETTE.accent) => ({ background: "transparent", border: `1px solid ${c}`, color: c, padding: "10px 20px", borderRadius: 8, cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: 600, letterSpacing: "0.04em", transition: "all .2s" }),
  btnFill: (c = PALETTE.accent) => ({ background: c, border: "none", color: "#000", padding: "11px 24px", borderRadius: 8, cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: 700, letterSpacing: "0.04em" }),
...
function CreativeThumb({ ad }) {
  ...
  if (!thumb || err) {
    ...
    return (
      <div style={{ width: 48, height: 48, background: `${PALETTE.border}`, borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>
        {icon}
      </div>
    );
  }
  return (
    <img src={thumb} alt="" onError={() => setErr(true)}
      style={{ width: 48, height: 48, objectFit: "cover", borderRadius: 6, flexShrink: 0, border: `1px solid ${PALETTE.border}` }} />
  );
}
        ...
        <div style={S.card}>
          ...
          {step === 1 && (
            <div style={{ fontSize: 13, color: PALETTE.muted, lineHeight: 1.8 }}>
              ...
              {[
                <>Andá a <a href="https://developers.facebook.com/tools/explorer" target="_blank" rel="noreferrer">Graph API Explorer</a></>,
                "Seleccioná tu app de Meta",
                "Clickeá Generate Access Token",
                <>Activá: <span style={S.tag(PALETTE.accent)}>ads_read</span> <span style={S.tag(PALETTE.purple)}>ads_management</span></>,
                <>El Ad Account ID está en la URL del Ads Manager: <code style={{ color: PALETTE.accent }}>?act=XXXXXXXXXX</code></>,
              ].map((s, i) => (
...
function OverviewTab({ campaigns }) {
  ...
  const summary = campaigns.reduce((acc, c) => ({ spend: acc.spend+c.spend, imp: acc.imp+c.imp, clicks: acc.clicks+c.clicks, purch: acc.purch+c.purch, v3: acc.v3+c.v3, tp: acc.tp+c.tp }), { spend:0,imp:0,clicks:0,purch:0,v3:0,tp:0 });
        ...
        <div style={{ ...S.card, flex: 1, minWidth: 220 }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <RadarChart data={[
              ...
              { m: "CPA", v: Math.max(0,100-campaigns.filter(c=>c.cpa>0).reduce((a,c) => a+c.cpa,0)/(campaigns.filter(c=>c.cpa>0).length||1)/500*100) },
              { m: "Freq", v: Math.max(0,100-(campaigns.reduce((a,c) => a+c.freq,0)/campaigns.length)*15) },
            ]}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
        {[
          ...
          <div key={title} style={{ ...S.card, flex: 1, minWidth: 280 }}>
            ...
            {list.map((a, i) => (
              <div key={i} onClick={() => setSelectedAd(a)} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10, cursor: "pointer", padding: "6px 8px", borderRadius: 8, transition: "background .15s" }}
                onMouseEnter={e => e.currentTarget.style.background="#ffffff08"}
                onMouseLeave={e => e.currentTarget.style.background="transparent"}>
                <CreativeThumb ad={a} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ color: PALETTE.text, fontSize: 12, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.name}</div>
                  <div style={{ color: PALETTE.muted, fontSize: 10, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.campaignName}</div>
                  <div style={{ color: PALETTE.muted, fontSize: 10, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>📁 {a.adsetName}</div>
                </div>
      ...
      {scatterData.length > 1 && (
        <div style={S.card}>
          <div style={S.label}>HOOK RATE vs HOLD RATE — tamaño = gasto, objetivo: arriba a la derecha</div>
      ...
      <div style={S.card}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
          <div style={S.label}>TODOS LOS ADS ({filtered.length})</div>
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            <span style={{ color: PALETTE.muted, fontSize: 11 }}>Min. gasto $</span>
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${PALETTE.border}` }}>
                {["Creativo","Ad","Conjunto","Campaña","Tipo","Gasto","CTR","CPM","Hook%","Hold%","Compras","ROAS","CPA"].map(h => (
                  <th key={h} style={{ color: PALETTE.muted, fontWeight: 600, padding: "8px 10px", textAlign: "left", whiteSpace: "nowrap" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map((a, i) => (
                <tr key={i} style={{ borderBottom: `1px solid ${PALETTE.border}22`, cursor: "pointer" }}
                  ...
                  onMouseEnter={e => e.currentTarget.style.background="#ffffff08"}
                  onMouseLeave={e => e.currentTarget.style.background="transparent"}>
                  <td style={{ padding: "8px 10px" }}><CreativeThumb ad={a} /></td>
                  <td style={{ padding: "8px 10px", color: PALETTE.text, maxWidth: 160, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={a.name}>{a.name}</td>
                  <td style={{ padding: "8px 10px", color: PALETTE.muted, maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={a.adsetName}>{a.adsetName}</td>
                  <td style={{ padding: "8px 10px", color: PALETTE.muted, maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={a.campaignName}>{a.campaignName}</td>
      ...
      {/* Ad detail modal */}
      {selectedAd && (
        <div onClick={() => setSelectedAd(null)} style={{ position: "fixed", inset: 0, background: "#00000088", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div onClick={e => e.stopPropagation()} style={{ ...S.card, maxWidth: 560, width: "100%", maxHeight: "90vh", overflowY: "auto" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
              <div style={{ fontWeight: 700, color: PALETTE.text, fontSize: 14, flex: 1, marginRight: 16 }}>{selectedAd.name}</div>
              <button onClick={() => setSelectedAd(null)} style={{ ...S.btn(PALETTE.muted), padding: "4px 10px", fontSize: 12 }}>✕</button>
            </div>
            {(selectedAd.thumbnailUrl || selectedAd.imageUrl) && (
              <img src={selectedAd.thumbnailUrl || selectedAd.imageUrl} alt="" style={{ width: "100%", borderRadius: 8, marginBottom: 16, border: `1px solid ${PALETTE.border}` }} />
            )}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
              {[
                ["Campaña", selectedAd.campaignName, PALETTE.muted],
                ["Conjunto", selectedAd.adsetName, PALETTE.muted],
                ["Tipo", selectedAd.creativeType, PALETTE.purple],
                ["Gasto", fm.$(selectedAd.spend), PALETTE.text],
                ["ROAS", selectedAd.roas>0 ? fm.x(selectedAd.roas) : "—", selectedAd.roas>=2 ? PALETTE.green : selectedAd.roas>=1 ? PALETTE.yellow : PALETTE.red],
                ["CPA", selectedAd.cpa>0 ? fm.$(selectedAd.cpa) : "—", PALETTE.purple],
                ["CTR", fm.pct(selectedAd.ctr), selectedAd.ctr>1.5 ? PALETTE.green : PALETTE.yellow],
                ["CPM", fm.$(selectedAd.cpm), PALETTE.muted],
                ["Hook Rate", fm.pct(selectedAd.hookRate), selectedAd.hookRate>30 ? PALETTE.green : PALETTE.yellow],
                ["Hold Rate", fm.pct(selectedAd.holdRate), selectedAd.holdRate>40 ? PALETTE.green : PALETTE.yellow],
                ["Compras", String(selectedAd.purch||"—"), PALETTE.green],
                ["ThruPlay", fm.n(selectedAd.tp), PALETTE.muted],
              ].map(([label, val, color]) => (
                <div key={label} style={{ background: "#0e0e1c", padding: "10px 12px", borderRadius: 8 }}>
                  <div style={S.label}>{label}</div>
                  <div style={{ color, fontWeight: 600, fontSize: 13, fontFamily: "DM Mono,monospace", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{val}</div>
                </div>
              ))}
            </div>
          </div>
  ...
  const run = async () => {
    ...
    try {
      ...
      const adBlock = list => list.map(a =>
        `• ${a.name} [${a.creativeType}]\n  Campaña: ${a.campaignName} | Conjunto: ${a.adsetName}\n  Gasto:${fm.$(a.spend)} CTR:${fm.pct(a.ctr)} CPM:${fm.$(a.cpm)} ROAS:${fm.x(a.roas)} CPA:${a.cpa>0?fm.$(a.cpa):"N/A"} Compras:${a.purch||0} Hook:${fm.pct(a.hookRate)} Hold:${fm.pct(a.holdRate)}`
      ).join("\n");
...
Gasto: ${fm.$(summary.spend)} | ROAS: ${fm.x(summary.roas)} | CPA: ${summary.cpa>0?fm.$(summary.cpa):"N/A"} | CTR: ${fm.pct(summary.ctr)} | CPM: ${fm.$(summary.cpm)} | Compras: ${summary.purch} | Hook Rate: ${fm.pct(summary.hookRate)}

CAMPAÑAS:
${campaigns.map(c => `• ${c.name} | ${c.objective} | ${c.status} | Gasto:${fm.$(c.spend)} ROAS:${fm.x(c.roas)} CPA:${c.cpa>0?fm.$(c.cpa):"N/A"} CTR:${fm.pct(c.ctr)} Hook:${fm.pct(c.hookRate)}`).join("\n")}

TOP PERFORMERS:
${adBlock(topAds)}

PEORES (con gasto significativo):
${adBlock(botAds)}

Analizá como el mejor media buyer del mundo. Respondé en español con estas secciones:

## 🎯 RESUMEN EJECUTIVO
(Estado de la cuenta, rentabilidad, urgencias)

## 📊 ANÁLISIS DE CAMPAÑAS
(Qué gana y por qué, qué pierde y por qué, con números específicos)

## 🎬 QUÉ ESTÁ FUNCIONANDO EN CREATIVOS
Identificá patrones en hooks, formatos, mensajes, ángulos de venta basándote en los nombres y métricas de los ads

## 👥 ANÁLISIS DE AUDIENCIAS / CONJUNTOS
(Qué conjuntos tienen mejor performance y qué tipo de buyer responde mejor)

## 📉 DIAGNÓSTICO DE LO QUE NO FUNCIONA
(Por qué fallan los peores ads — hook, CPM alto, CTR pésimo, fatiga de audiencia, etc.)

## 🚀 ITERACIONES CREATIVAS PARA TESTEAR
6-8 nuevos creativos concretos. Para cada uno: nombre sugerido, formato, hook propuesto (primeros 3 segundos), ángulo de venta, audiencia objetivo, por qué debería funcionar

## ⚡ ACCIONES INMEDIATAS
5 acciones concretas esta semana: qué pausar HOY, qué escalar HOY, qué testear, qué cambiar`;

      // Llamar al proxy en Vercel en vez de directamente a Anthropic
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {loading && <div style={S.card}><Loader text="Claude está analizando tu cuenta como senior media buyer..." /></div>}
...
export default function MetaAnalyzer() {
  ...
  const handleConnect = async (token, rawAccId, dp) => {
    ...
    setDatePreset(dp);
    const acct = await metaGet(`/${actId}`, token, { fields: "name,currency,account_status" });
    ...
    const adRes = await metaGet(`/${actId}/ads`, token, {
      fields: `name,status,adset_name,campaign{name},creative{name,object_type,thumbnail_url,image_url},insights.date_preset(${dp}){${insF}}`,
      limit: 500,
    });
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(ads.map(a => ({
      ...
      "ThruPlay": a.tp, "Add to Cart": a.addToCart,
    }))), "Creativos");
    ...
    <div style={{ minHeight: "100vh", background: PALETTE.bg, color: PALETTE.text, fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <div style={{ borderBottom: `1px solid ${PALETTE.border}`, padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: PALETTE.bg, zIndex: 100, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: PALETTE.muted }}>{accInfo?.name} · {datePreset.replace("last_","Últimos ").replace("d"," días").replace("this_month","Este mes").replace("last_month","Mes pasado")}</div>
        </div>
